...
std::vector<long long> run_parcels(int n, int k, int cores, const std::vector<std::vector<int>>& prefixes) {
    // Determine the number of concurrent workers. If cores is 0, use all available.
    unsigned int num_workers = (cores == 0) ? 
        std::thread::hardware_concurrency() : static_cast<unsigned int>(cores);
    if (num_workers == 0) num_workers = 4; // Fallback

    std::vector<long long> final_tally(2 * n, 0);

    // If there are fewer prefixes than workers, no need for complex logic.
    if (prefixes.size() < num_workers) {
    ...
    // --- Main logic for managing a pool of concurrent tasks ---
    std::vector<std::future<std::vector<long long>>> futures(num_workers);
    auto prefix_iter = prefixes.cbegin();

    // Start by filling the pool with the first batch of tasks.
    for (unsigned int i = 0; i < num_workers; ++i) {
        futures[i] = std::async(std::launch::async, [n, k, prefix = *prefix_iter++]() {
            return run_prefix<R>(n, k, prefix);
        });
    }

    // Process the remaining prefixes.
    while (prefix_iter != prefixes.cend()) {
        // Find a completed future in the pool to replace.
        // We use a simple round-robin check.
        for (unsigned int i = 0; i < num_workers; ++i) {
            // Check if a future is ready to be collected.
            if (futures[i].wait_for(std::chrono::seconds(0)) == std::future_status::ready) {
                // Get the result and add it to the final tally.
                auto tally = futures[i].get();
                for (int j = 0; j < 2 * n; ++j) {
                    final_tally[j] += tally[j];
                }

                // Replace the completed future with a new task.
                futures[i] = std::async(std::launch::async, [n, k, prefix = *prefix_iter++]() {
                    return run_prefix<R>(n, k, prefix);
                });
                
                // Break the inner loop once we've launched a new task.
                if (prefix_iter == prefixes.cend()) break;
            }
        }
        // If all futures were busy, sleep briefly to avoid a tight spin-loop.
        std::this_thread::sleep_for(std::chrono::microseconds(100));
    }

    // Wait for the last batch of tasks still in the pool to complete.
    for (auto& future : futures) {
        if (future.valid()) {
            auto tally = future.get();
            for (int i = 0; i < 2 * n; ++i) {
