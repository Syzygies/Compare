...
std::vector<long long> run_parcels(int n, int k, int cores, const std::vector<std::vector<int>>& prefixes) {
    ...
    if (cores == 0) {
        // cores = 0: use all hardware threads
        num_threads = std::thread::hardware_concurrency();
        if (num_threads == 0) num_threads = 4; // fallback if detection fails
    } else {
        // cores > 0: use exactly that many threads
        num_threads = cores;
    }
    
    // Each thread will accumulate its own tally
    std::vector<std::vector<long long>> thread_tallies(num_threads, std::vector<long long>(2 * n, 0));
    std::atomic<size_t> next_index(0);
    std::vector<std::thread> threads;
    
    auto worker = [&](int thread_id) {
        // Each thread gets its own Relations object and tally
        R rel;
        rel.create(2 * n);
        auto& my_tally = thread_tallies[thread_id];
        
        while (true) {
            size_t index = next_index.fetch_add(1);
            if (index >= prefixes.size()) break;
            
            // Process this prefix and accumulate in thread-local tally
            std::vector<int> perm = prefixes[index];
            auto work = [&](std::vector<int>& p) {
                tally_signs(n, p, my_tally, rel);
            };
            tally_perms(perm, k, work);
        }
    };
    ...
    // Merge all thread tallies into final result (no locks needed!)
    std::vector<long long> final_tally(2 * n, 0);
    for (const auto& thread_tally : thread_tallies) {
        for (int i = 0; i < 2 * n; ++i) {
            final_tally[i] += thread_tally[i];
        }
    }
